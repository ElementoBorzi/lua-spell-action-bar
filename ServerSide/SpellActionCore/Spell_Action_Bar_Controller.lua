local CONSTANT = {
    EVENT = {
        ...
        NAME = "Spell_Action_Bar_Verification_Event",
        DELAY = 5000,
    },
    DATA = "Spell_Action_Bar_Data",
    CLIENT = {
        SHOW_FRAME = "ShowFrame",
        HIDE_FRAME = "HideFrame",
        ADDON_NAME = "Spell_Action_Bar",
    }
}
...
local Entity = require("Spell_Action_Bar.Entities.Spell_Action_Bar_Entity")
...
function Spell_Action_Bar.CheckConditions(player, conditions)
...
function Spell_Action_Bar.HandleEvent(player, data)
    local verification_event = player:GetData(CONSTANT.EVENT.NAME)
    local player_data = player:GetData(CONSTANT.DATA)

    if Spell_Action_Bar.CheckConditions(player, data.conditions) then
        if (not verification_event) then
            local event_id = player:RegisterEvent(Spell_Action_Bar.Verification_Event, CONSTANT.EVENT.DELAY, 0)
...
function Spell_Action_Bar.Verification_Event(event_id, delay, repeats, player)
    local player_data = player:GetData(CONSTANT.DATA)
    local data = Entity.spells[player_data.spell_id]

    Spell_Action_Bar.HandleEvent(player, data)
end

function Spell_Action_Bar.OnItemEvent(event, player, item, count)
...
function Spell_Action_Bar.OnMapEvent(event, player)
...
function Spell_Action_Bar.OnZoneEvent(event, player, newZone, newArea)
...
function Spell_Action_Bar.OnAuraApply(event, player, aura)
...
function Spell_Action_Bar.OnAreaEvent(event, player, oldArea, newArea)
...
for event_id, event_name in pairs(CONSTANT.EVENT.PLAYER) do
    RegisterPlayerEvent(event_id, Spell_Action_Bar[event_name])
end
